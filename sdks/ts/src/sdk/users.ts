...
export class Users {
  ...
  **/
  postUsersTerminateSessions(
    req: operations.PostUsersTerminateSessionsRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.PostUsersTerminateSessionsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.PostUsersTerminateSessionsRequest(req);
    }
    
    const baseURL: string = this._serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/users/terminate_sessions";
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PostUsersTerminateSessionsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
