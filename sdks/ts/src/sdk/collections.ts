...
export class Collections {
  ...
  **/
  getCollections(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetCollectionsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  getCollectionsIdItems(
    req: operations.GetCollectionsIdItemsRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.GetCollectionsIdItemsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GetCollectionsIdItemsRequest(req);
    }
    
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/collections/{collection_id}/items", req.pathParams);
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetCollectionsIdItemsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
