...
func (s *Groups) PostGroupsTerminateSessions(ctx context.Context, request operations.PostGroupsTerminateSessionsRequest) (*operations.PostGroupsTerminateSessionsResponse, error) {
	baseURL := s._serverURL
	url := strings.TrimSuffix(baseURL, "/") + "/groups/terminate_sessions"
	...
	res := &operations.PostGroupsTerminateSessionsResponse{
		StatusCode:  int64(httpRes.StatusCode),
		ContentType: contentType,
	}
	switch {
	case httpRes.StatusCode == 202:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out *shared.SessionTerminationMessage
			if err := utils.UnmarshalJsonFromResponseBody(httpRes.Body, &out); err != nil {
				return nil, err
			}
